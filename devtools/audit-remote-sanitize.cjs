...
const forbiddenNames = [
  ...
  // 🔴 历史文件名「项目交接文档」—— 那份是**未脱敏**的原始交接稿，永远不许进仓库。
  //
  // ⚠️ 早先这里写的是 `/项目交接文档|项目文档/` + `/^docs\//` 两条，把
  //    `docs/项目文档.md`（**脱敏后**的单一完整版）也一起判成了危险文件 ——
  //    那条规则和 .gitignore 里的说明自相矛盾（.gitignore 明说
  //    "docs/项目文档.md 是脱敏后的版本，默认进 GitHub"），
  //    结果就是审计**永远失败**、失去闸门作用（人对一直红的灯会免疫）。
  //    所以收窄成：只拦**历史文件名**，docs/ 下的脱敏文档放行。
  //    —— 真实值仍然由下面的【B/C】内容扫描把关，那才是真正的防线。
  [/项目交接文档/, '未脱敏的历史交接文档'],
];
